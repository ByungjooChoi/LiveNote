...
- Zoom 최신 버전은 타일 AX 설명에 "active speaker" 토큰을 더 이상 노출하지 않는다(zoomtag.log 원문으로 확인). 1:1 회의는 "나 외 유일 타일" 폴백으로 상대방 이름을 붙인다. 그룹 회의 화자명은 Phase 3 Speaker Memory(오프라인 DiarizerManager 임베딩, v1.7.0)가 담당한다; 실제 회의로 threshold/margin 튜닝은 수동 QA 대상.
- 세션 WAV 폴더는 SessionAudioRecorder의 활성 폴더 레지스트리로 보호된다; purgeStale은 이 프로세스가 아직 쓰는 폴더를 지우지 않는다. 오프라인 diarizer, 라이브 임베딩 추출기, me 등록 임시 추출기는 각각 별도 OfflineDiarizer 인스턴스다(동기 엔진 호출이 actor를 점유하기 때문).
